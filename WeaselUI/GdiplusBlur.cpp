...
void DoGaussianBlur(Gdiplus::Bitmap* img, float radiusX, float radiusY) {
  ...
  Gdiplus::Bitmap temp(img->GetWidth(), img->GetHeight(), img->GetPixelFormat());
...
void DoGaussianBlurPower(Gdiplus::Bitmap* img,
  ...
  Gdiplus::Graphics g1(pBitmap);
  for (int i = 0; i < 8; ++i) {
    g1.DrawImage(pBitmap, 0, 0);
    if ((1 << i) & nPower) {
      Gdiplus::Graphics g2(img);
      g2.DrawImage(pBitmap, 0, 0);
    }
  }
  delete pBitmap;
}
