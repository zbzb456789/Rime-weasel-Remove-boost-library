...
int APIENTRY _tWinMain(HINSTANCE hInstance,
  ...
  int ret = 0;
  HANDLE hMutex = CreateMutex(nullptr, TRUE, L"WeaselDeployerExclusiveMutex");
  if (!hMutex) {
    ret = 1;
...
static int Run(LPTSTR lpCmdLine) {
  ...
  if (!wcscmp(L"/?", lpCmdLine) || !wcscmp(L"/help", lpCmdLine)) {
    WCHAR msg[1024] = {0};
    if (LoadString(GetModuleHandle(nullptr), IDS_STR_HELP, msg,
                   sizeof(msg) / sizeof(TCHAR))) {
      MessageBox(nullptr, msg, L"Weasel Deployer", MB_ICONINFORMATION | MB_OK);
    } else {
      MessageBox(nullptr,
