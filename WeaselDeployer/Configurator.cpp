...
void Configurator::Initialize() {
  ...
  static std::string shared_dir = wtou8(WeaselSharedDataPath().wstring());
  static std::string user_dir = wtou8(WeaselUserDataPath().wstring());
  weasel_traits.shared_data_dir = shared_dir.c_str();
  weasel_traits.user_data_dir = user_dir.c_str();
  weasel_traits.prebuilt_data_dir = weasel_traits.shared_data_dir;
  static std::string distribution_name = wtou8(get_weasel_ime_name());
  weasel_traits.distribution_name = distribution_name.c_str();
  weasel_traits.distribution_code_name = WEASEL_CODE_NAME;
  weasel_traits.distribution_version = WEASEL_VERSION;
  weasel_traits.app_name = "rime.weasel";
  static std::string log_dir = wtou8(WeaselLogPath().wstring());
  weasel_traits.log_dir = log_dir.c_str();
  RimeApi* rime_api = rime_get_api();
  assert(rime_api);
  rime_api->setup(&weasel_traits);
  LOG(INFO) << "WeaselDeployer reporting.";
  rime_api->deployer_initialize(nullptr);
}
...
int Configurator::UpdateWorkspace(bool report_errors) {
  HANDLE hMutex = CreateMutex(nullptr, TRUE, L"WeaselDeployerMutex");
  if (!hMutex) {
