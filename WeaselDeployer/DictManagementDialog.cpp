...
LRESULT DictManagementDialog::OnRestore(WORD, WORD code, HWND, BOOL&) {
  ...
  if (open_str.LoadStringW(IDS_STR_OPEN) == 0) {
    open_str = L"Open";
  }
  if (dict_snapshot_str.LoadStringW(IDS_STR_DICT_SNAPSHOT) == 0) {
    dict_snapshot_str = L"Dictionary Snapshot";
  }
  if (kcss_dict_snapshot_str.LoadStringW(IDS_STR_KCSS_DICT_SNAPSHOT) == 0) {
    kcss_dict_snapshot_str = L"KCSS Snapshot";
  }
  if (all_files_str.LoadStringW(IDS_STR_ALL_FILES) == 0) {
    all_files_str = L"All Files";
  }

  const std::wstring dict_snapshot_name =
      std::wstring(static_cast<LPCWSTR>(dict_snapshot_str)) + L" (*.userdb.txt)";
  const std::wstring kcss_dict_snapshot_name =
      std::wstring(static_cast<LPCWSTR>(kcss_dict_snapshot_str)) + L" (*.userdb.kct.snapshot)";
  const std::wstring all_files_name = std::wstring(static_cast<LPCWSTR>(all_files_str));
...
LRESULT DictManagementDialog::OnExport(WORD, WORD code, HWND, BOOL&) {
  CString save_as_str, exported_str, record_count_str, all_files_str,
      txt_files_str;
  if (save_as_str.LoadStringW(IDS_STR_SAVE_AS) == 0) save_as_str = L"Save As";
  if (exported_str.LoadStringW(IDS_STR_EXPORTED) == 0) exported_str = L"Exported";
  if (record_count_str.LoadStringW(IDS_STR_RECORD_COUNT) == 0) record_count_str = L"records";
  if (txt_files_str.LoadStringW(IDS_STR_TXT_FILES) == 0) txt_files_str = L"Text Files";
  if (all_files_str.LoadStringW(IDS_STR_ALL_FILES) == 0) all_files_str = L"All Files";
  const std::wstring txt_files_name = std::wstring(static_cast<LPCWSTR>(txt_files_str)) + L" (*.txt)";
  const std::wstring all_files_name = std::wstring(static_cast<LPCWSTR>(all_files_str));
...
LRESULT DictManagementDialog::OnImport(WORD, WORD code, HWND, BOOL&) {
  CString open_str, imported_str, record_count_str, all_files_str,
      txt_files_str;
  if (open_str.LoadStringW(IDS_STR_OPEN) == 0) open_str = L"Open";
  if (imported_str.LoadStringW(IDS_STR_IMPORTED) == 0) imported_str = L"Imported";
  if (record_count_str.LoadStringW(IDS_STR_RECORD_COUNT) == 0) record_count_str = L"records";
  if (txt_files_str.LoadStringW(IDS_STR_TXT_FILES) == 0) txt_files_str = L"Text Files";
  if (all_files_str.LoadStringW(IDS_STR_ALL_FILES) == 0) all_files_str = L"All Files";

  const std::wstring txt_files_name =
      std::wstring(static_cast<LPCWSTR>(txt_files_str)) + L" (*.txt)";
  const std::wstring all_files_name =
      std::wstring(static_cast<LPCWSTR>(all_files_str));
