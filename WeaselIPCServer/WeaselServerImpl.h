...
// class ServerImpl
{
 ...
 public:
  ...
  void AddMenuHandler(UINT uID, CommandHandler handler) {
    m_MenuHandlers.emplace(uID, std::move(handler));
  }

 private:
  void _Finailize();
  template <typename _Resp>
  void HandlePipeMessage(const PipeMessage& pipe_msg, const _Resp& resp);

  std::unique_ptr<PipeServer> channel;
  std::unique_ptr<std::jthread> pipeThread;
  RequestHandler* m_pRequestHandler;  // reference
  std::unordered_map<UINT, CommandHandler> m_MenuHandlers;
