...
class wbufferstream : public std::wiostream {
  struct wbuf : std::wstreambuf {
    wbuf(wchar_t* b, size_t sz) {
      setp(b, b + sz);
      setg(b, b, b + sz);
    }
  } buf;
public:
  wbufferstream(wchar_t* b, size_t sz) : std::wiostream(&buf), buf(b, sz) {}
};

class PipeChannelBase {
 public:
  using Stream = weasel::wbufferstream;
