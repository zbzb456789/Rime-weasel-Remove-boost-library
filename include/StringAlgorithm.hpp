...
inline std::vector<std::wstring>& split(std::vector<std::wstring>& result, std::wstring_view input, const wchar_t* delim)
{
	result.clear();
	size_t current = 0;
	size_t next = std::wstring_view::npos;
	do
	{
		current = next + 1;
		next = input.find_first_of(delim, next + 1);
		result.emplace_back(input.substr(current, next - current));
