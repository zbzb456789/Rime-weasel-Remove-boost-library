...
struct KeyEvent {
  ...
  // 使用显式赋值来初始化两个位域，替换原来的 reinterpret_cast 写法
  KeyEvent(UINT x) {
    keycode = static_cast<UINT>(x & 0xFFFFu);
    mask = static_cast<UINT>((x >> 16) & 0xFFFFu);
  }

  operator UINT32 const() const {
    return (static_cast<UINT32>(mask) << 16) | static_cast<UINT32>(keycode);
  }
};
}  // namespace weasel
