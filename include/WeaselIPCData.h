...
class text_woarchive {
  std::wostream& os;
public:
  text_woarchive(std::wostream& os) : os(os) {}

  text_woarchive& operator<<(const std::wstring& s) {
    os << s.length() << L" " << s << L" ";
    return *this;
  }
  text_woarchive& operator<<(bool b) {
    os << (b ? 1 : 0) << L" ";
    return *this;
  }
  template <typename T, std::enable_if_t<std::is_enum_v<T> || std::is_integral_v<T>, int> = 0>
  text_woarchive& operator<<(T v) {
    os << static_cast<int64_t>(v) << L" ";
    return *this;
  }

  template <typename T>
  text_woarchive& operator<<(const std::vector<T>& v) {
    os << v.size() << L" ";
  ...
  template <typename T, std::enable_if_t<!std::is_enum_v<T> && !std::is_integral_v<T>, int> = 0>
  text_woarchive& operator<<(const T& v) {
    serialize(*this, const_cast<T&>(v), 0);
    return *this;
  }
...
class text_wiarchive {
  std::wistream& is;
public:
  text_wiarchive(std::wistream& is) : is(is) {}

  text_wiarchive& operator>>(std::wstring& s) {
    size_t len;
    if (!(is >> len)) throw archive_exception("Failed to read string length");
    is.ignore(1); // space
    if (len > 0) {
      s.resize(len);
      is.read(s.data(), len);
  ...
  text_wiarchive& operator>>(bool& b) {
    int v;
    if (!(is >> v)) throw archive_exception("Failed to read bool");
    b = (v != 0);
    return *this;
  }
  template <typename T, std::enable_if_t<std::is_enum_v<T> || std::is_integral_v<T>, int> = 0>
  text_wiarchive& operator>>(T& v) {
    int64_t val;
    if (!(is >> val)) throw archive_exception("Failed to read int");
    v = static_cast<T>(val);
    return *this;
  }
  ...
  text_wiarchive& operator>>(std::vector<T>& v) {
    size_t len;
    if (!(is >> len)) throw archive_exception("Failed to read vector length");
    v.resize(len);
