...
// 在 include\wtl\atlapp.h 中替换原来的实现
class CStaticDataInitCriticalSectionLock {
 public:
  ...
  // 返回 HRESULT，成功时将内部标志置为 true
  HRESULT Lock() {
    HRESULT hr = m_cslock.Lock();
    if (SUCCEEDED(hr))
      m_bLocked = true;
    return hr;
  }

  // 仅在内部标志为 true 时才解锁，并重置标志
  void Unlock() {
    if (m_bLocked) {
      m_cslock.Unlock();
      m_bLocked = false;
...
public:
	...
	CWindowCreateCriticalSectionLock() : m_cslock(ATL::_AtlWinModule.m_csWindowCreate, false), m_bLocked(false) {}
