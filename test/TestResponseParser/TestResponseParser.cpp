...
#define BOOST_TEST(expr) \
  if (!(expr)) { \
    std::cerr << __FILE__ << "(" << __LINE__ << "): test '" << #expr << "' failed." << std::endl; \
    ++g_test_errors; \
  }

#define BOOST_ASSERT(expr) BOOST_TEST(expr)
#define BOOST_TEST_EQ(expr1, expr2) BOOST_TEST((expr1) == (expr2))
