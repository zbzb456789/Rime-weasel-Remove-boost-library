...
bool launch_server() {
  int ret = static_cast<int>(reinterpret_cast<INT_PTR>(ShellExecute(nullptr, L"open", L"TestWeaselIPC.exe", L"/start",
                              nullptr, SW_NORMAL)));
  if (ret <= 32) {
...
bool read_buffer(LPWSTR buffer, UINT length, LPWSTR dest) {
  std::wistringstream bs(std::wstring(buffer, length));
...
const char* wcstomb(const wchar_t* wcs) {
  ...
  WideCharToMultiByte(CP_OEMCP, 0, wcs, -1, buffer, buffer_len, nullptr, nullptr);
  return buffer;
}
 ...
 public:
  ...
  virtual BOOL ProcessKeyEvent(weasel::KeyEvent keyEvent,
    ...
    std::cerr << "ProcessKeyEvent: " << session_id
              ...
              << std::endl;
    std::wstring greet = L"Greeting=Hello, 小狼毫.\n";
    eat(greet);
    return TRUE;
  }
...
int server_main() {
  HRESULT hRes = _Module.Init(nullptr, GetModuleHandle(nullptr));
  ATLASSERT(SUCCEEDED(hRes));

  weasel::Server server;
  // weasel::UI ui;
  // const auto handler = std::make_unique<RimeWithWeaselHandler>(&ui);
  const auto handler = std::make_unique<TestRequestHandler>();
