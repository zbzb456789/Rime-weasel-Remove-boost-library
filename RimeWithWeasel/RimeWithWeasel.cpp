...
void RimeWithWeaselHandler::_Setup() {
  ...
  std::string log_dir = wtou8(WeaselLogPath().wstring());
...
void RimeWithWeaselHandler::_ReadClientInfo(WeaselSessionId ipc_id,
  ...
  // parse request text
  wbufferstream bs(buffer, wcsnlen(buffer, 32760));
  std::wstring line;
  while (bs.good()) {
    std::getline(bs, line);
    if (!bs.good())
...
void RimeWithWeaselHandler::OnUpdateUI(const std::function<void()>& cb) {
...
bool RimeWithWeaselHandler::_Respond(WeaselSessionId ipc_id, EatLine eat) {
  ...
  if (rime_api->get_context(session_id, &ctx)) {
    ...
    if (has_candidates) {
      ...
      weasel::archive::text_woarchive oa(ss);
