...
STDAPI CGetTextExtentEditSession::DoEditSession(TfEditCookie ec) {
  ...
  // 1. 正常通过 TSF 获取坐标
  HRESULT hr = _pContextView->GetTextExt(ec, pRange, &rc, &fClipped);

  bool isFallbackUsed = false;

  // 2. 兜底方案：解决 CAD 无法获取坐标，以及首个字母短暂 (0,0) 的问题
  if (FAILED(hr) || (rc.left == 0 && rc.top == 0)) {
    POINT pt = {0, 0};

    // 方案 A：尝试获取系统的原生输入光标 (Caret)
    if (::GetCaretPos(&pt) && (pt.x != 0 || pt.y != 0)) {
      ...
      rc.top = pt.y;
      rc.right = rc.left;
      rc.bottom = rc.top + 10;  // 必须赋予高度
      hr = S_OK;
      isFallbackUsed = true;
    }
    // 方案 B：如果连原生光标都没有（例如 CAD 绘图区），直接用鼠标位置
    else if (::GetCursorPos(&pt)) {
      rc.left = pt.x + 10;  // 稍微往右下方偏移一点，避免完全挡住鼠标/十字线
      rc.top = pt.y + 10;
      rc.right = rc.left;
      rc.bottom = rc.top + 10;
      hr = S_OK;
  ...
  if (SUCCEEDED(hr) && (rc.left != 0 || rc.top != 0)) {
    ...
    // window 注意：如果使用了鼠标兜底位置，就不应该再应用 enhanced_position
    // 修正，防止偏移错乱
    if (_enhancedPosition && !isFallbackUsed) {
      HWND hwnd;
    ...
    // 绝对防线：只要坐标不是纯粹的 (0,0)，才发送更新，从而彻底消灭左上角闪现
...
void WeaselTSF::_SetCompositionPosition(const RECT& rc) {
  ...
  if (!_fCUASWorkaroundTested) {
    ...
    // 修复点：高度为0时，不再 return
    // 丢弃坐标（这会导致第一笔被吃掉从而闪现/卡死）
    // 而是赋予一个默认高度，强行让后续更新生效
    if (safeRc.top == safeRc.bottom) {
      _fCUASWorkaroundEnabled = TRUE;
      safeRc.bottom = safeRc.top + 10;
    }
  }

  // 终极保险：如果送过来的坐标仍旧没有高度，给它高度，防止除以零或渲染异常
  if (safeRc.top == safeRc.bottom) {
    safeRc.bottom = safeRc.top + 10;
  }

  m_client.UpdateInputPosition(safeRc);
  _cand->UpdateInputPosition(safeRc);
}
...
STDAPI CInlinePreeditEditSession::DoEditSession(TfEditCookie ec) {
  ...
  int sel_cursor = -1;
  for (const auto& attr : _context->preedit.attributes) {
    if (attr.type == weasel::HIGHLIGHTED) {
      sel_cursor = attr.range.cursor;
      break;
    }
  }
